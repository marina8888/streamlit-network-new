def run():
    ...
    # User inputs
    mg_per_m3 = st.sidebar.number_input("Concentration (mg/m³)", value=100.0)
    molecular_weight = st.sidebar.number_input("Molecular Weight (g/mol)", value=44.01)  # e.g., CO2
    temperature = st.sidebar.number_input("Temperature (K)", value=298.15)
    pressure = st.sidebar.number_input("Pressure (Pa)", value=101325.0)
    energy_mj = st.sidebar.number_input("Energy Output (MJ)", value=10.0)
